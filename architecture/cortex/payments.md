...
- While negative, manual model selection is lost (pickers close, sends are rewritten to Dynamic Chat) and media is refused server-side; at or below the floor nothing is sendable until the allowance renews. The chat briefing overlay is the single credit-warning surface: from zero down to the floor it warns that Cortex's intelligence drops (the server caps the lanes), and at or below the floor it switches to the usage-limit message. It dismisses on tap or downward swipe and stays hidden until the credit state actually changes; the old always-on credit banner is gone.
