...
class _AiBodyContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final thinkMatch = _thinkRegex.firstMatch(fullText);
    ...
    if (thinkMatch != null) {
      ...
      if (mainStable.contains(matchedStr)) {
         ...
         mainStable = mainStable.replaceAll(_thinkRegex, '').trimLeft();
         mainAnimating = mainAnimating.replaceAll(_thinkRegex, '');
      }
              ...
              TextSpan(
                ...
                children: [
                  ...
                  if (markdownText.animating.isNotEmpty)
                    ..._getAnimatingSpans(context, markdownText.animating, s, opacity),
                ],
              ),
            );
          },
  ...
  List<InlineSpan> _getParsedSpans(
    ...
    final cacheKey =
        '$text:${message.isThinking}:$citationKey:$s:$colorKey';
