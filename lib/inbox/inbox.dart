...
class MenuScreenState extends State<MenuScreen>
  ...
  void initState() {
    ...
    // Load from cache using the new optimized CacheService ---
    ...
    final cachedStarred = CacheService.get<List<String>>(CacheKey.starredIds);
    final cachedUserModels = CacheService.get<List<Map<String, dynamic>>>(CacheKey.userModels);

    // Load from cache only if the essential data (managers and order) exists.
  ...
  void didChangeDependencies() {
    ...
    if (!_isInitialSetupComplete) {
      ...
      // If the cache was empty during initState, our local _conversationManagers map will also be empty.
      if (_conversationManagers.isEmpty) {
        _loadConversations();
      }
  ...
  void _listenForLastMessageUpdates() {
    ...
    _lastMessageSubscription = ChatStorageService.lastMsgStream.listen((update) {
      final String convId = update['convId'];
      final String newText = update['text'];
      final String newPhotoPath = update['photoPath'] ?? '';
      final DateTime newDate = DateTime.fromMillisecondsSinceEpoch(update['ts']);

      final manager = _conversationManagers[convId];
      if (manager != null) {
        manager.updateLastMessage(newText, newPhotoPath, newDate);
  ...
  void _updateConversationCache() {
    ...
    CacheService.set(CacheKey.conversationOrder, List.of(_conversationIDsOrder));
    CacheService.set(CacheKey.starredIds, List.of(_starredConversationIDs));
    CacheService.set(CacheKey.userModels, List.of(_userModels));
    debugPrint("[MenuScreen] Conversation cache updated with new data.");
  }
