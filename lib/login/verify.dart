...
class _EmailVerificationScreenState extends State<EmailVerificationScreen> {
  ...
  Future<void> _continueToApp() async {
    ...
    try {
      dev.log(
          '[Continue] User chose to continue without verification. Navigating to MainScreen.',
          name: 'EmailVerification');
      ...
      dev.log('[Continue] Unexpected error during navigation: $e',
          name: 'EmailVerification', error: e, stackTrace: st);
      if (mounted) {
  ...
  Future<void> _navigateToMainScreenAfterVerification() async {
    ...
    dev.log(
        '[EmailVerification] Running purchase reconciliation after successful verification.',
        name: 'EmailVerification');
  ...
  Future<void> _resendVerificationEmail() async {
    ...
    try {
      ...
      dev.log('Incremented verifyAttempts to ${verifyAttempts + 1}.',
          name: 'EmailVerification');
      ...
      if (e.code == 'too-many-requests') {
        ...
        _notificationService.showNotification(
            message: '${l10n.authError}: ${e.message}',
            type: NotificationType.error,
            bottomOffset: 0.02);
      }
    } catch (e) {
