...
class OfflineModeratorService {
  static final OfflineModeratorService _instance =
    ...
    try {
      await _remoteConfig.setConfigSettings(RemoteConfigSettings(
        ...
        minimumFetchInterval:
            const Duration(hours: 1), // Fetch updates max once per hour
      ));
      await _remoteConfig.fetchAndActivate();
      _buildAndCompileRegex();

      _configSub = _remoteConfig.onConfigUpdated.listen((event) async {
        await _remoteConfig.activate();
        _buildAndCompileRegex();
        debugPrint("[OfflineModerator] Rules updated and re-compiled.");
      });
      _configSub?.onError((error) {
        debugPrint(
            "[OfflineModerator] Config stream error ignored safely: $error");
      });
    } catch (e) {
  ...
  void _buildAndCompileRegex() {
    ...
    if (jsonString.isEmpty) {
      debugPrint(
          "[OfflineModerator] Warning: Blacklist from Remote Config is empty.");
      _compiledRegex = null;
      return;
    }
    ...
    try {
      ...
      final List<String> exactWords =
          List<String>.from(config['exactWords'] ?? []);
      if (exactWords.isNotEmpty) {
        patterns.add(r'\b(' + exactWords.join('|') + r')\b');
      }

      // Add word roots, allowing for suffixes.
      final List<String> wordRoots =
          List<String>.from(config['wordRoots'] ?? []);
      ...
      if (phrases.isNotEmpty) {
        patterns.add(
            '(${phrases.map((p) => p.replaceAll(' ', r'\s+')).join('|')})');
      }
  ...
  bool isPromptAcceptable(String prompt) {
    if (_compiledRegex == null) {
      debugPrint(
          "[OfflineModerator] Regex not available. Allowing prompt as a fallback.");
    ...
    if (_compiledRegex!.hasMatch(prompt)) {
      debugPrint(
          "[OfflineModerator] Prompt rejected by on-device moderation rule.");
      return false;
    }
