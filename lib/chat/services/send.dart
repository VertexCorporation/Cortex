...
class SendService {
    ...
    for (final model in allModels) {
      if (model.variants?.isNotEmpty ?? false) {
        for (final entry in model.variants!.entries) {
          ...
          final source =
              variantData['source']?.toString().toLowerCase() ??
          ...
          final precise = _modelService.getPreciseModelData(
            variantId,
            langCode: langCode,
          );
  ...
  Future<bool> sendMessage({
    ...
    // [VoiceLoop] boundary evidence (debug-only): the committed voice turn
    // reached SendService. Gated on the live voice/flow flags so ordinary
    ...
    if (isRegenerate) {
      ...
      currentAttachmentPaths = isContinue
          ? []
          : List.from(lastUserMessage.attachmentPaths);
    } else {
      // Use the class member _inputProvider or the local one.
      // Since we injected it, _inputProvider is safe.
      currentAttachmentPaths = _inputProvider.attachments
          .map((a) => a.file.path)
          .toList();
    }
    ...
    Future<void> handleServerTitle(String title) async {
      ...
      final rawTitle = title.length > 40 ? title.substring(0, 40) : title;
    ...
    try {
      ...
      if (isContinue) {
        ...
        textForApi = "Continue your previous response exactly where it stopped. Resume mid-sentence without repeating any earlier text, and finish the response.";
      }
      ...
      if (apiModelIdForSend != null && apiModelIdForSend != 'cortex/auto') {
        ...
        if (entity.variants != null && entity.variants!.isNotEmpty) {
          ...
          final bool hasVisualContent = currentAttachmentPaths.any(
            (path) => _mediaRouter.isImageFile(path),
          );
      ...
      final selectedModelForSnapshot = _modelService.getPreciseModelData(
        originalUiModelId,
        langCode: langCode,
      );
      final modelTitleForStorage = selectedModelForSnapshot.displayTitle;
      final modelImagePathForStorage = _modelService.getModelImagePath(
        selectedModelForSnapshot,
      );
      ...
      if (isRegenerate && regenerateAiIndex != null) {
              ...
              _apiService
                  ...
                  .then((aiTitle) async {
                    if (serverTitleReceived) {
                      debugPrint(
                        "[TitleGen] Server-side title already received, skipping client-side title.",
                      );
                      return;
                    }
                    if (aiTitle != null && aiTitle.trim().isNotEmpty) {
                      final rawTitle = aiTitle.length > 40
                          ? aiTitle.substring(0, 40)
                          : aiTitle;
                      ...
                      final didRename =
                          await ChatStorageService.renameConversation(
                            newConvId,
                            cleanTitle,
                            source: 'titlegen',
                      ...
                      if (_conversationProvider.conversationID == newConvId) {
                        _conversationProvider.updateConversationTitle(
                          cleanTitle,
                        );
                      }
                    }
        ...
        // [VoiceLoop] boundary evidence (debug-only): the user bubble exists
        // in the conversation — the boundary right before the AI request.
        if (kDebugMode &&
            (_inputProvider.isVoiceModeActive || _voiceService.isFlowActive)) {
        ...
        Future<void> switchToDynamicFallback({
          ...
          if (isContinue) {
            ...
            if (_isConversationActive(convId)) {
              ...
              _conversationProvider.updateMessageAtIndex(
                aiMessageIndex,
                currentMsg.copyWith(model: 'cortex/auto'),
              );
            }
          } else {
        ...
        while (attempt < 3 && !success) {
          ...
          try {
            ...
            if (e is ApiException && e.code == 'EMPTY_RESPONSE') {
              ...
              final hasImageAttachment = currentAttachmentPaths.any(
                _mediaRouter.isImageFile,
              );
              final canRetryImageToImage =
                  hasImageAttachment &&
          ...
          if (aiMessageIndex >= 0 && aiMessageIndex < messages.length) {
            ...
            if (imageTagRegex.hasMatch(botText)) {
              ...
              if (imageModels.isNotEmpty) {
                ...
                final freeImageModels = imageModels
                    .where((m) => !m.isPremium)
                    .toList();
                ...
                _conversationProvider.updateMessageAtIndex(
                  aiMessageIndex,
                  messages[aiMessageIndex].copyWith(
                    text: localizations.errorNoModelsAvailable,
                  ),
                );
              }
            }
      ...
      // -----------------------------------------------------------------------
      if (!isAutoRouter) {
        ChatStorageService.addRecentModel(
          apiModelIdForSend!,
          langCode: langCode,
          modelService: _modelService,
        ).ignore();
  ...
  Future<void> _sendServerSideMessageWithLoop({
    ...
    if (kDebugMode &&
        (_inputProvider.isVoiceModeActive || _voiceService.isFlowActive)) {
      debugPrint('[ChatRequest] started model=$modelId continue=$isContinue');
    }

    final ModelEntity modelData = _modelService.getPreciseModelData(
      modelId,
      langCode: langCode,
    );
    ...
    List<Map<String, dynamic>> contextMessages = await _contextService
        ...
        if (modelData.category == 'image' && contextMessages.isNotEmpty) {
          ...
          if (lastMsg['role'] == 'user') {
            ...
            if (originalText != null && originalText.isNotEmpty) {
              ...
              onMediaGenerating('image');
              final optimizedPrompt = await _apiService.optimizeImagePrompt(
                originalText,
              );
    ...
    final bufferedResponseText = _backgroundTaskService.peekBuffer(
      targetConvId,
    );
    String finalResponseText = bufferedResponseText;
    bool hasGeneratedMedia = _backgroundTaskService
        .getMediaAttachments(targetConvId)
        .isNotEmpty;
    ...
    if (memoryMatch != null) {
      ...
      if (newMemory != null && newMemory.isNotEmpty) {
        ...
        debugPrint(
          '[Memory] Lifecycle: extracted ${lines.length} fact(s) '
  ...
  void _handleSendError(
    ...
    if (error is ApiException) {
      ...
      final String? deterministicCopy = creditRefusalRecoveryMessage(
        ...
        renewalRemaining: credits.nextDailyRenewal().difference(DateTime.now()),
        localizations: localizations,
      );
      if (deterministicCopy != null && !isRegenerate) {
        _recoverCreditRefusal(
          ...
          failedAttachmentPaths: failedAttachmentPaths ?? const <String>[],
          generationTarget: generationTarget,
          langCode: langCode,
        );
        return;
  ...
  Future<void> _attachGeneratedMediaToAiMessage({
    ...
    if (mediaPath.startsWith('http://') || mediaPath.startsWith('https://')) {
      debugPrint(
        '[SendService] MEDIA PERSISTENCE WARNING: generated media '
      ...
      debugPrint(
        '[SendService] Generated media attached (conv='
        '$targetConvId, index=$aiMessageIndex, '
    ...
    if (_isConversationActive(targetConvId)) {
      ...
      final updatedAttachments = List<String>.from(
        currentAiMessage.attachmentPaths,
      );
  ...
  void _syncActiveMessageFromBackgroundBuffer(
    ...
    final backgroundAttachments = _backgroundTaskService.getMediaAttachments(
      convId,
    );
  ...
  Future<void> _persistBackgroundCompletion(
    ...
    try {
      ...
      final mediaAttachments = _backgroundTaskService.getMediaAttachments(
        convId,
      );
      ...
      Message? existingMsg;
      try {
        existingMsg = await ChatStorageService.getMessageAtIndex(
          convId,
          aiMessageIndex,
        );
      } catch (_) {}
      ...
      debugPrint(
        '[SendService] Background completion persisted for $convId '
...
String? creditRefusalRecoveryMessage({
  ...
  if (normalizedCode == null || !_creditRefusalCodes.contains(normalizedCode)) {
    return null;
  }
  ...
  final String renewal = formatRenewalRemainingLocalized(
    renewalRemaining,
    localizations,
  );
