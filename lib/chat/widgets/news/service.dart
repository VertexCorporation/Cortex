...
class NewsService with ChangeNotifier {
  ...
  void _onConnectivityChanged() {
    if (_internetProvider.isConnected && _languagesWithFetchErrors.isNotEmpty) {
      debugPrint("[NewsService] 📶 Internet reconnected. Retrying failed languages...");
      final bool shouldRetry = _languagesWithFetchErrors.contains(_currentLanguageCode);
  ...
  Future<void> loadNewsForLanguage(String languageCode) async {
    ...
    // Memory Cache Check
    if (_cachedArticlesByLang.containsKey(languageCode) && (_cachedArticlesByLang[languageCode]?.isNotEmpty ?? false)) {
    ...
    try {
      ...
      debugPrint("[NewsService] 🏁 Operation finished (Success or Error). Resetting flag.");
  ...
  Future<void> _fetchFromNetworkAndCache() async {
    ...
    while (attempts < maxAttempts) {
      try {
        attempts++;
        debugPrint("[NewsService] 📡 Network Attempt $attempts/$maxAttempts started.");
        ...
        if (signedUrl == null) throw Exception("Cloud Function returned NULL signedUrl.");
  ...
  void _handleError(String message, Object e, StackTrace s) {
    debugPrint('[NewsService] 💥 FINAL ERROR for "$_currentLanguageCode": $message -> $e');
  ...
  void _setState(NewsState newState, String source) {
    ...
    debugPrint("[NewsService] 🔔 State: $newState ($source). Count: ${articles.length}");
    _state = newState;
    notifyListeners();
  }
}