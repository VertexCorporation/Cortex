...
class CreateScreenState extends State<CreateScreen> with TickerProviderStateMixin {
  ...
  Future<void> _saveModel() async {
    ...
    final HttpsCallable authCallable = FirebaseFunctions.instanceFor(region: 'europe-west1')
        .httpsCallable('createCustomModel', options: HttpsCallableOptions(timeout: const Duration(seconds: 30)));

    try {
      await _imageFileToBase64(_pickedImage);
      await authCallable.call({ 'modelType': 'roleplay', 'name': _nameController.text.trim(), /* ... other params */ });
      ...
      final Map<String, dynamic> modelData = {
        'id': modelId, 'title': _nameController.text.trim(), 'summary': _summaryController.text.trim(),
        'description': _modelExplanationController.text.trim(), 'role': _aiPromptController.text.trim(),
        'baseModelId': _selectedBaseModelId, 'imagePath': imagePath, 'type': 'roleplay', 'category': 'self',
        'producer': '_USER_', 'createdAt': DateTime.now().toIso8601String(),
      };
      await dbHelper.insert('models', {
        ...
        conflictAlgorithm: ConflictAlgorithm.replace, userId: user.uid,
      );
      ...
      final newModelEntity = ModelEntity.fromMap(modelData, langCode);
      ModelData.addModelToCache(newModelEntity);
  ...
  void _initializeDefaultBaseModel() {
    final allEntities = widget.availableBaseModels;

    // Convert the list of entities back to a list of maps, ONLY for the
    // call to the raw data handler utility 'findDefaultBaseModel'.
    final modelMaps = allEntities.map((e) => e.toMap()).toList();
    final defaultId = ModelData.findDefaultBaseModel(modelMaps);

    if (defaultId != null) {
      final langCode = Localizations.localeOf(context).languageCode;
      final modelEntity = ModelData.getPreciseModelData(defaultId, langCode: langCode);

      if (mounted) {
        setState(() {
          _selectedBaseModelId = defaultId;
          _selectedBaseModelDisplayTitle = modelEntity.displayTitle;
  ...
  Widget _buildBaseModelSection() {
    ...
    // Filter the entities directly using their safe properties.
    final availableBaseModels = widget.availableBaseModels
        .where((model) => model.isServerSide && (model.extensions?.isNotEmpty ?? false))
        .toList();
    ...
    if (_selectedBaseModelId != null) {
      for (var series in availableBaseModels) {
        if (series.extensions?.containsKey(_selectedBaseModelId) ?? false) {
          selectedVariantData = series.extensions![_selectedBaseModelId];
          break;
        }
      }
    }
    final bool isCurrentlySelectedPremium = (selectedVariantData?['tier'] as String? ?? 'free') == 'premium';
          ...
          appBar: AppBar(
            ...
            actions: [
              Opacity(
                ...
                child: IconButton(
                  icon: SvgPicture.asset('assets/icons/transition.svg', width: screenWidth * 0.06, height: screenWidth * 0.06, colorFilter: ColorFilter.mode(AppColors.primaryColor.inverted, BlendMode.srcIn)),
                  ...
                  onPressed: (_isSubscriptionLoading || _isSaving) ? null : () async {
                    ...
                    if (shouldPopImmediately == true && mounted && navigator.canPop()) {
                      navigator.pop();
                    }
                  },
                ),
                  ...
                  child: GestureDetector(
                    ...
                    child: AnimatedOpacity(
                      ...
                      child: AnimatedRotation(
                        ...
                        child: SvgPicture.asset('assets/icons/plus.svg', width: avatarSize * 0.2, height: avatarSize * 0.2, colorFilter: ColorFilter.mode(AppColors.primaryColor.inverted, BlendMode.srcIn)),
                      ),
                    ),
                  ),
                ),
