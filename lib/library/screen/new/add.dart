...
class AddScreenState extends State<AddScreen> with TickerProviderStateMixin {
  ...
  Future<void> _saveModel() async {
    ...
    try {
      ...
      _cache.clearAddData();
      // Create a ModelEntity from the raw map data.
      final newModelEntity = ModelEntity.fromMap(modelData, langCode);

      // Pass the new, type-safe entity to the cache.
  ...
  Future<void> _pickGgufFile() async {
    if (_isFilePickerActive) {
      debugPrint("File picker is already active. Ignoring tap.");
      return;
    }
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        if (filePath != null && filePath.toLowerCase().endsWith('.gguf')) {
          ...
          notificationService.showNotification(
              message: localizations.errorGGUF, isSuccess: false);
        }
      }
    } catch (e) {
      debugPrint("Error during file picking: $e");
    } finally {
      if (mounted) setState(() => _isFilePickerActive = false);
    }
  }
          ...
          child: Container(
            ...
            child: Center(
              child: _ggufFile != null
                ...
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  SvgPicture.asset('assets/icons/upload.svg', width: screenWidth * 0.1, height: screenWidth * 0.1, colorFilter: ColorFilter.mode(AppColors.primaryColor.inverted, BlendMode.srcIn)),
                  SizedBox(height: screenHeight * 0.005),
