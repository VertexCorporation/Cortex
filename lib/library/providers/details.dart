...
class ModelDetailProvider extends ChangeNotifier {
  ...
  // --- Dynamic Getters ---
  bool get isDownloaded => _localStateProvider.downloadCompleted[_modelId] ?? false;
  ...
  bool get isPremiumModelSelected => _currentCapabilitiesSource?.isPremium ?? false;
  ...
  Future<void> _initialize() async {
    dev.log("[ModelDetailProvider] Initializing for ID '$_modelId'", name: 'ModelDetail');
  ...
  Future<void> _loadAndProcessData() async {
    ...
    _mainModel = _modelService.getPreciseModelData(_modelId, langCode: langCode);

    if (_mainModel == null) {
      dev.log("[ModelDetailProvider] CRITICAL: Could not load main model entity for ID '$_modelId'", name: 'ModelDetail');
      return;
    }
  ...
  void dispose() {
    _isDisposed = true;
    dev.log("[ModelDetailProvider] Disposing for ID '$_modelId'", name: 'ModelDetail');
  ...
  Future<void> selectBaseModel(BuildContext context, String newBaseModelId) async {
    ...
    try {
      ...
      final success = await _modelService.updateBaseModel(_modelId, newBaseModelId);
      if (!success) {
        _notificationService.showNotification(message: localizations.anErrorOccurred, type: NotificationType.error);
        return;
      }
      ...
      dev.log("[ModelDetailProvider] Error changing base model: $e", name: 'ModelDetail');
      _notificationService.showNotification(message: localizations.anErrorOccurred, type: NotificationType.error);
    } finally {
  ...
  Future<bool> removeModel(BuildContext context) async {
    ...
    try {
      ...
      dev.log("[ModelDetailProvider] Error during model removal call: $e", name: 'ModelDetail');
  ...
  void _onUserStatusChanged() {
    ...
    if (_isUserSubscribed != newSubscriptionStatus) {
      ...
      dev.log("[ModelDetailProvider] User subscription status changed to: $_isUserSubscribed", name: 'ModelDetail');
      notifyListeners();
    }
  }
  ...
  void _processData(AppLocalizations localizations) {
    ...
    displayProducer = _mainModel!.producer == '_USER_' ? localizations.you : _mainModel!.producer;
  ...
  void _updateSourceEntities(String langCode) {
    ...
    if (isCharacterModel) {
      selectedBaseModel = (selectedBaseModelId != null && selectedBaseModelId!.isNotEmpty)
          ? _modelService.getPreciseModelData(selectedBaseModelId!, langCode: langCode)
          : null;
  ...
  void _updateComputedDisplayStrings(AppLocalizations localizations) {
    ...
    if (isPluralModel) {
      ...
      if (displaySummary.trim().isEmpty) displaySummary = selectedBaseModel?.displaySummary ?? '';
      if (displayDescription.trim().isEmpty) displayDescription = selectedBaseModel?.displayDescription ?? '';
    } else {
      displaySummary = _mainModel!.displaySummary;
      displayDescription = _mainModel!.displayDescription;
    }
    ...
    displayModality = (dataEntity?.modalities['image'] == true) ? localizations.multimodal : localizations.text;
  }
  ...
  List<String> _parseFeaturesData() {
    ...
    if (_mainModel!.category == 'roleplay' || _mainModel!.category == 'self') features.add('roleplay');
